function [accuracySummary,heads] = projectNB(param, extractFeatures)
...
display 'SVM classifier training'

if (param.crossval==0)
     Mdl = fitcnb(X,Y,...             % Naive Bayes built-in function
    'Distribution', 'mn',...         % Decide which distribution to use
    'Crossval','off',...             % Decide if to use cross-validation or not
    'Classnames',{'1','0','-1'}) ;   % Set classnames
else
    CVMdl = fitcnb(X,Y,...           % Naive Bayes built-in function
    'Distribution', 'mn',...         % Decide which distribution to use
    'Crossval','on',...              % Decide if to use cross-validation or not
    'Classnames',{'1','0','-1'}) ;   % Set classnames
